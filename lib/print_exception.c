...
void print_exception_infos(
    exc_frame_t *frame, const char *exception_name, unsigned int exception_source_addr,
    bool is_data_abort, bool is_prefetch_abort,
    unsigned int data_fault_status_register,
    unsigned int data_fault_address_register,
    unsigned int instruction_fault_status_register,
    unsigned int instruction_fault_address_register,
...
void print_psr(unsigned int psr)
{
    ...
    kprintf("%c%c%c%c %c %c%c%c", psr_n ? 'N' : '_', psr_z ? 'Z' : '_', psr_c ? 'C' : '_',
            psr_v ? 'V' : '_', psr_e ? 'E' : '_', psr_i ? 'I' : '_', psr_f ? 'F' : '_',
            psr_t ? 'T' : '_');
...
const char *get_fsr_description(unsigned int fsr)
{
    ...
    if (fsr_status > sizeof(fsr_sources) / sizeof(const char *) || fsr_sources[fsr_status] == NULL) {
