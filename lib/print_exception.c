...
void print_exception_infos(exc_frame_t *frame, const char *exception_name,
			   unsigned int exception_source_addr, bool is_data_abort,
			   bool		is_prefetch_abort,
			   unsigned int data_fault_status_register, // DFSR
			   unsigned int data_fault_address_register, // DFAR
			   unsigned int instruction_fault_status_register, // IFSR
			   unsigned int instruction_fault_address_register, // IFAR
			   unsigned int cpsr, unsigned int irq_spsr, unsigned int abort_spsr,
			   unsigned int undefined_spsr, unsigned int supervisor_spsr)
{
	...
	if (is_data_abort) {
		const char *dfsr_desc = get_fsr_description(data_fault_status_register);
	...
	if (is_prefetch_abort) {
		const char *ifsr_desc = get_fsr_description(instruction_fault_status_register);
		kprintf("Instruction Fault Status Register: 0x%08x -> %s\n",
			instruction_fault_status_register, ifsr_desc);
...
// Extract PSR flags and print exactly like example
void print_psr(unsigned int psr)
{
	...
	kprintf("%c%c%c%c %c %c%c%c %s 0x%08x", psr_n ? 'N' : '_', psr_z ? 'Z' : '_',
		psr_c ? 'C' : '_', psr_v ? 'V' : '_', psr_e ? 'E' : '_', psr_i ? 'I' : '_',
		psr_f ? 'F' : '_', psr_t ? 'T' : '_', mode_name, psr);
}
