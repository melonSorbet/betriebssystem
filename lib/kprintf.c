...
static int int_to_str(int value, char* buffer) {
    int len = 0;
    unsigned int uval = value;
    if (value < 0) uval = -value;
...
static int uint_to_hex_str(unsigned int value, char* buffer) {
    ...
    while (value > 0) {
        int digit = value % 16;
        if (digit < 10) buffer[len++] = '0' + digit;
        else buffer[len++] = 'a' + (digit - 10);
        value /= 16;
    }
...
static void send_int_width(int value, int width, char pad_char) {
    char buf[12];
...
static void send_unsigned_width(unsigned int value, int width, char pad_char, bool hex) {
    char buf[12];
    int len;
    if (hex) len = uint_to_hex_str(value, buf);
    else len = int_to_str(value, buf); // for %u
...
static void send_pointer(void* ptr) {
        ...
        if (*fmt == '%') {
            ...
            switch (*fmt++) {
                ...
                case 'i':
                    ...
                    break;
                case 'u':
                    send_unsigned_width(va_arg(args, unsigned int), width, pad_char, false);
                    break;
                case 'x':
                    send_unsigned_width(va_arg(args, unsigned int), width, pad_char, true);
                    break;
                case 'p':
